...
def reserBook(ID,ISBN,Date):
    db.child("books").child(ISBN).update({"reserved":"yes"})
    data = {"ID":ID,"ISBN":ISBN,"Date":Date}
    db.child("inreserved").child(ID).push(data)

def returnreserve(ID,ISBN,Date):
    db.child("books").child(ISBN).update({"reserved":"no"})
    db.child("inreserve").child(ID).remove()

def booksonreserve():
    books = db.child("books").get()
    for person in books.each():
        if person.val()["reserved"] =="yes":
            print(person.val())
            print(person.key())


def viewreserve():
    loans = db.child("inreserve").get()
    for person in loans.each():
        print(person.val())
        print(person.key())
